...
2. 담당자 한 명이 공식 API 키를 넣어 학기 전체 공개 시간표를 받는다.
...
일반 교사는 API 키와 원본 파일을 보지 않는다. 교사별 시간표 파일을 쓸 수 있으면 설정 목표는
...
브라우저 프로토타입에서는 이용자 브라우저에만 보관한다.

### 10.3 실데이터와 합성 데이터의 경계
...
공식 API 키를 발급받은 연구 환경에서 다음 코퍼스를 만든다.
...
- `Lesson`: 날짜, 교시, 학급, 과목, 담당 교사 또는 미배정 상태
...
8개 데모 사건을 각각 시작부터 게시 또는 안전한 중단까지 자동화한다. 실제 관측 사례와 연결된
...
1. 8개 현실 데모가 데스크톱과 모바일에서 자동으로 통과한다.
