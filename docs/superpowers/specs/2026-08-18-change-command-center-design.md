...
P0 승인 항목 14개를 모두 자동 검사로 덮었다. 근거는 `npm test` 396개(엔진 160, 웹 236)와
`npm run smoke` 다. smoke 는 진입과 설정, 교사 여정, 접근성과 키보드, 핵심 여정 5개, 운영
실패 3종을 1440, 390, 320 세 폭에서 잰다.

| 승인 항목 | 상태 | 근거 |
|---|---|---|
| 학교 검색, 예시 진입, 일회성 설정 | 완료 | `neis.test.ts`, 진입과 설정 smoke |
| 공식 자료 완전성과 진단 차단 | 완료 | 엔진 정규화 시험, `neis.test.ts`, 총계 불일치 흐름 |
| 교사 오늘과 주간 시간표, 변경 전후 | 완료 | `teacher-schedule-view.test.ts`, 교사 smoke |
| 단일 교시와 하루 부재 사건 | 완료 | `case-service.test.ts`, 여정 1과 2 |
| 행 기반 교체, 연쇄, 보강 비교 | 완료 | `resolution.test.ts`, 비교 표 smoke |
| 운영 변경 관제판 | 완료 | `ops-command-center.test.ts`, 관제판 smoke |
| 대안 선택, 승인, 반려 | 완료 | 전이 시험, 여정 1과 4 |
| 사건 전체 충돌과 낡은 후보 재검증 | 완료 | `revalidation.test.ts`, 여정 4 |
| 나이스 입력, 공지, 기록, 게시 마감 | 완료 | `publication.test.ts`, `publication-center.test.ts`, 여정 1 |
| 게시 후에만 교사와 학급으로 전파 | 완료 | 게시 전후를 새 페이지로 여는 여정 1과 5 |
| 내부 자료가 새지 않는 학급 공개 시간표 | 완료 | 투영 시험, `npm run check` 경계 검사, 여정 전체 |
| 사용자 여정과 자료 품질 회귀 | 완료 | 시연 시나리오 10개, 핵심 여정 5개 |
| 저장, 네트워크, 부분 자료 실패 | 완료 | `repository.test.ts`, 운영 실패 흐름 3종, 클립보드 거부 |
| 데스크톱, 모바일, 키보드, 접근성 | 완료 | Axe 6화면 0건, 키보드 완주, 세 폭 조작 크기 |

현장 리허설 전에 남은 위험은 [계획서](../plans/2026-08-18-change-command-center-prototype.md)
말미와 작업 13 보고서에 적었다. 단계 B 와 C 는 이 문서에 접어 넣지 않고 새 설계로 연다.
...
저장소의 코퍼스 매니페스트에는 아래 검증된 집계와 문서에 값이 남은 세 학교의 관측만 보존한다.
원본 대량 응답과 학교별 조회 매니페스트는 커밋되지 않았으므로 12개 학교의 이름·코드·조회 주간·
학교별 건수를 역산하거나 만들어 내지 않는다. 이 필드는 `unavailable`로 표시하고, 자격 증명이 있는
연구 환경에서 코퍼스를 다시 조회한 뒤에만 갱신한다. 공개 회귀 픽스처는 키 없이 최소 원시 행 증거를
제공하며, 전체 응답 캐시는 저장하지 않는다.
...
운영 시나리오 1~8의 날짜·교시·과목·교실도 관측 구조를 조합한 합성 일정이며 특정 나이스 응답의
정확한 행이라고 표시하지 않는다. 화면에는 `공개 시간표 관측 구조 기반 · 일정·교사·사건은 예시`라고
표시하고 시간표 버전 출처를 `demo`로 둔다. 정확한 공식 공개 행과 픽스처 경로는 자료 진단 시나리오
9~10에서만 사용한다. 두 종류 모두 교사 배정과 운영 사건 출처는 `synthetic-demo`로 분리한다.
