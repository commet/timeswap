 ...
 * 실제 자료의 과목명은 중앙 5자, 95퍼센타일 10자, 99퍼센타일 14자다.
 * **최대는 35자다.** 앞서 "최대 14자"라 적었던 것은 99퍼센타일이었다.
 * 20자를 넘는 과목이 있는 학교가 고등학교 217곳 가운데 20곳, 특수학교 32곳 가운데 2곳이다.
 *
 * | 학교급 | 중앙 | p95 | p99 | 최대 |
 * |---|---|---|---|---|
 * | 초등학교 | 2 | 7 | 7 | 16 |
 ...
 * | 특수학교 | 5 | 10 | 17 | 33 |
 *
 * 그래서 예시 학교에 26자짜리를 넣어 둔다. 폰 화면을 재는 검사가 쉬운 것만 재면
 * 실제 학교에서 글자가 잘린다.
...
const SAMPLE_SUBJECTS: ReadonlyArray<readonly [string, number, string?]> = [
  ...
  ['성공적인 직업생활', 3, '직업생활'],
  ['디지털 논리 회로', 3, '논리회로'],
  ['로봇 주제어장치(MCU) 하드웨어 회로 설계', 3, '부품선정'],
  ['휠·타이어·얼라인먼트 정비', 2, '정비'],
  ['진로활동', 2, '진로'],
  ['자율·자치활동', 2, '자치'],
];
