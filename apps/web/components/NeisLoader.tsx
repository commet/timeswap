...
export function NeisLoader({ neisKey, onKeyChange, onDone, onCancel }: Props) {
    ...
    return (
      <div className="neis">
        ...
        <label className="neis-field">
          ...
          />
          {/*
           * 발급 절차를 확인해 적었다. 링크를 열면 회원가입 화면이 아니라 로그인 화면이고
           * 카카오톡, 네이버, 구글 계정으로 바로 들어간다. 포털이 그 자리에
           * "복잡한 회원가입 절차없이" 라고 적어 두었다.
           * 이 사실이 빠져 있으면 "발급"이라는 말만 보고 관공서 회원가입을 떠올려 접는다.
           */}
          <span className="neis-hint">
            나이스는 키 없는 요청에 <b>5줄만</b> 보냅니다. 학교 하나가 수천 줄이라 키가
            필요합니다.
            <br />
            <b>회원가입은 없습니다.</b> 아래를 열어 <b>카카오톡이나 네이버 계정으로 로그인</b>한 뒤
            인증키 신청을 누르시면 바로 나옵니다.{' '}
  ...
  return (
    <div className="neis">
      ...
      {/*
       * 이 화면이 이 도구에서 가장 무거운 자리다.
       * 실제 특성화고를 재어 보니 (학급, 과목) 짝이 223개였다. 같은 학년 같은 과목을
       * 한 번에 채우는 단추를 쓰면 손으로 치는 횟수가 62회로 줄지만 그래도 적지 않다.
       *
       * 중요한 것은 이 일을 학교에서 한 사람만 하면 된다는 사실이다. 채운 결과를 파일로
       * 내보내 동료에게 주면 동료는 열기만 하면 된다. 그 말을 여기 적어야
       * 모두가 각자 223칸을 채우려 들지 않는다.
       */}
      <p className="neis-lede">
      ...
      <p className="neis-share">
        <b>이 일은 학교에서 한 분만 하시면 됩니다.</b> 채운 뒤 위쪽 <b>파일로 저장</b>으로
        내보내 동료께 전달하시면, 받으신 분은 <b>저장한 파일 열기</b>로 바로 시작하십니다.
        인증키도 그분들께는 필요하지 않습니다.
      </p>
