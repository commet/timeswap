# 수업품앗이

학기 중 수업 교환을 찾아 주는 도구. `timeswap` 은 저장소 이름으로만 남은 가칭이다.

교사 출장, 연수, 병가로 생기는 결강을 학교는 "교체 우선, 실패 시 보강" 규정으로 처리한다.
교체 탐색이 수작업이라 포기가 잦고, 포기는 보강 수당과 수업 결손으로 이어진다.
이 도구는 성립하는 교환안을 전부 찾아 순위와 근거를 함께 보여 준다.

## 지금 되는 것

- 결강 수업을 누르면 맞바꾸기, 연쇄 교환, 빈 시간 옮기기 후보를 전수 탐색해 순위로 제시
- 후보마다 통과한 조건과 감점, 가점을 문장으로 표시
- 하루 결근 대기열, 요일 통째 결강, 근무 불가 시간 잠금
- 시간표에 실제 반영, 되돌리기, 변경 장부
- 요청 문구 복사, 변경 공지 복사, 나이스 입력용 목록 복사, 수업 교체 계획서 인쇄
- 교환이 없을 때 그 교시에 비어 있는 교사 목록 제시
- 나이스 교육정보 개방 포털에서 학교를 찾아 학급 시간표와 학사일정을 직접 불러오기
- 품앗이 균형: 이미 도와준 교사는 다음 추천에서 뒤로 밀림

## 자료 취급 원칙

1. 다른 회사의 자료와 코드를 쓰지 않는다. 기능이 무엇인지는 조사해 우리 문장으로 적되 옮겨 적지 않는다.
2. 입력은 공적 자료이거나 학교 자신의 것만 받는다. 다른 서비스의 내부 응답에 기대지 않는다.
3. 시간표는 이용자 브라우저에만 둔다. 우리 서버는 없다.
4. 나이스 호출은 이용자 브라우저가 직접 하고, 인증키도 이용자 것을 쓴다.

## 구조

```
packages/engine   프레임워크에 매이지 않는 탐색 엔진 (TypeScript)
  src/search.ts     전수 탐색: 이동, 2자 맞교환, 3자 연쇄
  src/score.ts      소프트 점수와 근거 트레이스
  src/cover.ts      교환이 없을 때의 보강 후보
  src/synthetic.ts  구성적 학교 생성기 (이분 그래프 간선 색칠)
  src/adapters/neis.ts  나이스 개방 자료 해석과 변경 이력 복원
apps/web          Next.js 정적 내보내기
  lib/neis.ts       나이스 개방 포털 클라이언트
```

## 문서

| 문서 | 내용 |
|---|---|
| [00-research.md](00-research.md) | 딥 리서치: 국내 현황, 해외 제품, 알고리즘 문헌, 시장과 구매 구조 |
| 01-plan.md | 제품과 구현 계획 v1. 아직 [PR #1](https://github.com/commet/timeswap/pull/1) 초안이라 main 에 없다 |
| [02-spike-record.md](02-spike-record.md) | 스파이크 기록. 데이터 경로 판단은 03 에서 뒤집혔다 |
| [03-domain-and-data.md](03-domain-and-data.md) | 운영 실태, 나이스 개방 API 실측, 자료 취급 원칙, 분반 교환 |
| [04-operations.md](04-operations.md) | 배포 구조, 검증 관문, 의존성 정책, 데이터베이스 판단 |

## 개발

```bash
npm ci              # 잠금 파일 그대로 설치
npm run typecheck   # 타입 검사
npm test            # 엔진 시험 49개
npm run build       # 웹 정적 빌드 (결과는 apps/web/out)
npm run demo        # 엔진 CLI 데모
```

배포와 같은 조건으로 확인하려면 응답 헤더를 씌워 띄운다.

```bash
node scripts/serve-checked.mjs   # vercel.json 의 헤더를 그대로 적용
npm run smoke                    # 다른 창에서 브라우저 전 과정 점검
```

푸시와 풀 리퀘스트마다 GitHub Actions 가 위 전부를 다시 돌린다.
자세한 구성은 [04-operations.md](04-operations.md) 에 있다.

## 다음 단계

1. 분반과 이동수업 묶음의 교환 탐색
2. 학사일정 휴업일과 학년 행사를 탐색에 반영
3. 나이스 교사별 시간표 내보내기 파일 읽기
4. 현직 교사 인터뷰로 관문 통과 확인
