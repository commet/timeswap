...
export interface TimetableInput {
  ...
  /**
   * 학급별로 그 요일에 실제로 있는 교시 수.
   *
   * `config.periodsPerDay` 는 학교 전체로 하나다. 그런데 학년마다 하루 교시 수가
   * 다른 학교가 많다. 초등학교 1학년은 4교시에 끝나고 6학년은 6교시까지 한다.
   * 학교 전체로 6교시라고 두면 1학년의 5, 6교시가 빈 시간으로 보이고,
   * 거기로 옮기라는 안이 나온다. 그 교시는 그 학년에 없는 시간이다.
   *
   * 실측에서 같은 요일에 학년 간 교시 수가 2교시 이상 벌어지는 학교가
   * 초등학교 57곳 가운데 54곳, 특수학교 32곳 가운데 25곳이었다.
   * 막기 전에는 초등학교 53곳에서 그 학년에 없는 교시로 옮기라는 안이 나왔다.
   *
   * 학급이 하나뿐인 학년에는 넣지 않는다. 그저 비어 있는 칸과 가릴 수 없어서다.
   */
  klassPeriodsPerDay?: Record<string, number[]>;
